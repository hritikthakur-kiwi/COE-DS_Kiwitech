...
import { getAllTrending, deleteTrendingByID } from '../../../apiHelper';

export default function UseCaseList() {
const [tending, setTending] = useState([]);
const deleteAction = async (id) => {
  const result = await deleteTrendingByID(id);
  alert(result);
}
      ...
      <main>
        <nav>
            <ul className='ul-style'>
                <li> <h1>All Trending</h1></li>
                <li><Link className='button-style' href="/admin/trending/add">Add</Link></li>
            </ul>
        </nav>
        <div className="cards">
          {tending?.map((one) => (
            <div key={one._id} className="card">
              <Link href={`/admin/trending/edit/${one._id}`}>
                  <h2>{one.name}</h2>
